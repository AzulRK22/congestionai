...
function Heatmap({ data }: { data: { hourOfWeek: number; risk: number }[] }) {
  ...
  return (
    <div
      className="grid gap-0.5 border p-2 rounded"
      style={{ gridTemplateColumns: "repeat(24, minmax(0,1fr))" }}
    >
      {cells.map((c, idx) => {
        const intensity = Math.round(c.risk * 100); // 0..100
        return (
          <div
            key={idx}
...
export default function Page() {
  ...
  async function plan() {
    ...
    const res = await fetch("/api/analyze", {
      ...
      body: JSON.stringify({ origin, destination: dest, window: "next72" }),
    });
  ...
  function copyWhatsApp() {
    if (!data?.best) return;
    const t = new Date(data.best.departAtISO).toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
    });
  ...
  return (
    <main className="max-w-3xl mx-auto p-6 space-y-4">
      ...
      <p className="text-sm text-gray-600">
        “Te digo cuándo NO manejar” — demo 1 clic
      </p>
      <input
      ...
      />
      <button onClick={plan} className="btn">
        {loading ? "Calculando…" : "Planear"}
      </button>
      ...
      {data && (
        <section className="card space-y-3">
          <div className="text-lg font-semibold">
            Sal{" "}
            {new Date(data.best.departAtISO).toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
            })}{" "}
            · ETA {data.best.etaMin} min · Ahorro{" "}
            {(data.best.savingVsNow * 100) | 0}%
          </div>
          <div className="flex gap-2">
            <button onClick={addICS} className="border rounded px-3 py-1">
              Add to Calendar
            </button>
            <button onClick={copyWhatsApp} className="border rounded px-3 py-1">
              Copy WhatsApp
            </button>
          </div>
          <div>
